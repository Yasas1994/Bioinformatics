...
file1 = read_file('D1.fasta')
file2 = read_file('D2.fasta')
file3 = read_file('D3.fasta')
file4 = read_file('D4.fasta')
file5 = read_file('t3.txt')
file6 = read_file('Zika.fasta')
test = []
file1_file2_bias = []
...
dengue = file1_file2_bias
type_lable = [0 for i in range(10)]
type_lable.extend([1 for i in range(10)])
type_lable.extend([2 for i in range(10)])
type_lable.extend([3 for i in range(10)])
type_lable.extend([4 for i in range(10)])
#print(zika_dengue[1:3])
#print(type_lable)
#fit the svm model
'''
model = svm.SVC(gamma='scale', decision_function_shape='ovr')
model.fit(dengue,type_lable)
'''
model = svm.LinearSVC()
model.fit(dengue,type_lable)
...
def Dengue(bias):
      if (model.predict(bias))==0:
            print ("prediction : Dengue Virus 1")
      if (model.predict(bias))==1:
            print ("prediction : Dengue Virus 2")
      if (model.predict(bias))==2:
            print ("prediction : Dengue Virus 3")
      if (model.predict(bias))==3:
            print ("prediction : Dengue Virus 4")
      if (model.predict(bias))==4:
            print ("prediction : Zika Virus 4")
Dengue(test)
'''
print(test)
'''
